...
bool Bomb::initWithType(int type)//不同炮弹类型
{
	if (!Sprite::init())
	{
		return false;
...
void Bomb::removeBullet()//类比arrow同名函数
{
	...
	for (int j = 0; j < monsterVector.size(); j++)
	{
		...
		if(monster!=NULL && bombPostion.distance(monsterPosition)<= 50  && monster->getAttackBySoldier())//炮弹落地是有范围的
		{
